...
var_types = {
    ...
    "BooleanType": {
        "type": "Char",
        "length": 5,
    },  # http://www.datypic.com/sc/xsd/t-xsd_boolean.html, legal values = [0,1,'true', 'false']
    ...
    "IntegerNNType": {
        "type": "Integer",
        "length": 15,
    },  # max value is unclear, see xsd:Integer vs xsd:int
    ...
    "DateType": {
        "type": "Char",
        "length": 31,
    },  # unclear http://www.datypic.com/sc/xsd/s-datatypes.xsd.html
    ...
    "SSNType": {
        "type": "Char",
        "length": 12,
    },  # should be 9 but needs to fit "XXX-XX-XXXX" which has 11
...
def get_django_type(vartype):
    ...
    if thisvar["type"] == "Integer":
        ...
        return (
            "DecimalField(null=True, blank=True, max_digits=%s, decimal_places=%s)"
            % (thisvar["totalDigits"], thisvar["fractionDigits"])
        )
...
if __name__ == "__main__":
    for key in var_types.keys():
        print(
            "key %s resolve to '%s' and '%s'"
            % (key, get_django_type(key), get_sqlalchemy_type(key))
        )
